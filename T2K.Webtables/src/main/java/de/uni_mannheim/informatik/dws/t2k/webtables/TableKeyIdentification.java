    ...
    public void identifyKeys(Table table) {
        ...
        for (int i = 0; i < table.getSchema().getSize(); i++) {
            ...
            if(isVerbose()) {
            	...
            	System.out.println(String.format("[TableKeyIdentification] [%d]%s (%s) Uniqueness=%.4f; Nullness=%.4f; Combined=%.4f; Length=%.4f", i, c.getHeader(), c.getDataType(), uniqueness, nullness, columnUniqueness.get(columnUniqueness.size()-1), columnValueLength.get(columnValueLength.size()-1)));
            }
        }
        ...
        if (key != null) {
            ...
            if (columnUniqueness.get(keyColumnIndex) >= getKeyUniquenessThreshold()
            	...
            	table.setKeyIndex(keyColumnIndex);

            	if(isVerbose()) {
            		System.out.println(String.format("[TableKeyIdentification] RegEx Header Match: '%s'", table.getSchema().get(keyColumnIndex).getHeader()));
            	}
            ...
            if(isVerbose()) {
            	System.out.println(String.format("[TableKeyIdentification] RegEx Header Match: '%s' - insufficient", table.getSchema().get(keyColumnIndex).getHeader()));
            }
        }

        if (columnUniqueness.isEmpty()) {
        	if(isVerbose()) {
        		System.out.println("[TableKeyIdentification] no columns");
        	}
            return;
        }
        double maxCount = -1;
        ...
        if (key == null) {
            if (maxColumn == -1) {
            	if(isVerbose()) {
            		System.out.println("[TableKeyIdentification] no columns that match criteria (data type, min length, max length)");
            	}
                return;
            }
        ...
        if (columnUniqueness.get(keyColumnIndex) < getKeyUniquenessThreshold()) {
        	...
        	if(isVerbose()) {
        		System.out.println(String.format("[TableKeyIdentification] Most unique column: '%s' - insufficient (%.4f)", table.getSchema().get(keyColumnIndex).getHeader(), columnUniqueness.get(keyColumnIndex)));
        	}
        ...
        if(isVerbose()) {
        	System.out.println(String.format("[TableKeyIdentification] Most unique column: '%s'", table.getSchema().get(keyColumnIndex).getHeader()));
        }
        table.setKeyIndex(keyColumnIndex);
    }
