...
import de.uni_mannheim.informatik.dws.t2k.datatypes.ColumnType;
import de.uni_mannheim.informatik.dws.t2k.datatypes.DataType;
import de.uni_mannheim.informatik.dws.t2k.datatypes.TypeConverter;
import de.uni_mannheim.informatik.dws.t2k.datatypes.TypeGuesser;
...
public class Table implements Serializable {
	...
	public void removeColumn(TableColumn c) {
		if(keyIndex!=-1 && c.getColumnIndex()<= keyIndex) {
			keyIndex--;
		} else if(c.getColumnIndex()==keyIndex) {
			keyIndex=-1;
		}
    	...
    	try {
			Parallel.forLoop(0, getSchema().getSize(), new Consumer<Integer>() {
				...
				public void execute(Integer i) {
			    	HashMap<TableRow, Unit> detectedUnit = new HashMap<>();
			    	
			    	// parse units from headers
			    	Unit columnUnit = UnitParser.parseUnitFromHeader(getSchema().get(i).getHeader());
			    	...
			    	if(getSchema().get(i).getDataType()==DataType.unknown) {
				    	...
				    	for(TableRow r : getRows()) {		    		
				    		...
				    		if(value!=null) {
				    			cdt = tg.guessTypeForValue(value, columnUnit);
				    		}
				    		
				    		if(cdt!=null) {
					    		MapUtils.increment(typeCount, cdt.getType());
					    		detectedUnit.put(r, cdt.getUnit());
				    		}
				    	}
				    	...
				    	getSchema().get(i).setDataType((DataType)type);
				    	if((DataType)type==DataType.unit) {
				    		getSchema().get(i).setDataType(DataType.numeric);
				    		getSchema().get(i).setUnit(detectedUnit.get(i));
				    	}
			    	}
				}
			});
	...
	public void convertValues() {
		...
		for(TableRow r : getRows()) {
			...
			for(int i=0; i<getSchema().getSize(); i++) {
				...
				r.set(i, tc.typeValue((String)r.get(i), getSchema().get(i).getDataType(), getSchema().get(i).getUnit()));
	 ...
	 * Detects and sets the key column for this table
	 */
	public void identifyKey(double uniquenessThreshold) {
		identifyKey(uniquenessThreshold, false);
	}
	
	public void identifyKey(double uniquenessThreshold, boolean verbose) {
		if(hasKey()) {
			// reset existing key
			setKeyIndex(-1);
		}
	...
	/**
	 * Appends the rows of the given table to this table's rows. Both tables must have the same schema!
	 * @param t
	 */
	...
	/**
	 * re-assigns the row numbers to rows. After rows are removed from the table, the row numbers no longer reflect the position of the row in the table, which can be corrected by calling this method.
	 */
	...
	public Table project(Collection<TableColumn> projectedColumns) throws Exception {
		...
		if(columnIndexProjection.containsKey(getKeyIndex())) {
			result.setKeyIndex(columnIndexProjection.get(getKeyIndex()));
		}
	...
	public Table copySchema() {
		...
		// copy path and key index
		result.setKeyIndex(getKeyIndex());
		result.setPath(getPath());
	...
	/**
	 * 
	 * Removes duplicates from the table. The provided key is used to find the duplicates. If duplicates are found, the first record matching the key is kept, all others are removed.
	 * 
	 * @param key
	 */
	...
	/**
	 * 
	 * Removes duplicates from the table. The provided key is used to find the duplicates. If duplicates are found, the remaining values are checked for conflicts and the ConflictHandling is applied:
	 * ConflictHandling.KeepFirst: The first record is kept, all others are removed
	 * ConflictHandling.KeepBoth: All conflicting records are kept
				...
				if(conflictHandling!=ConflictHandling.KeepFirst) {
					...
					for(TableColumn c : Q.without(getColumns(), key)) {
						...
						if(existingValue!=null && existingValue.equals(duplicateValue)) {
							// equal values
						} if(existingValue==null && duplicateValue!=null || existingValue!=null && duplicateValue==null) {
					...
					if(!equal){ 
						...
						if(conflictHandling==ConflictHandling.KeepBoth || conflictHandling==ConflictHandling.ReplaceNULLs && !conflictingNullsOnly) {
							// if handling is set to keep both we don't merge
							// if handling is set to replace nulls, but there is a conflict between non-null values, we don't merge
							continue;
						} else {
							// if handling is set to replace nulls, and there are only conflicts between values and nulls, we set the values in the existing record and remove the second record 
