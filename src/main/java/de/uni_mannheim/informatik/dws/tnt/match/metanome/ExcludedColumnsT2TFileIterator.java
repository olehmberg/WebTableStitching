 ...
 *
 */
public class ExcludedColumnsT2TFileIterator implements RelationalInput {
	...
	public ExcludedColumnsT2TFileIterator(String relationName, Reader reader,
			ConfigurationSettingFileInput setting, Collection<Integer> excludedColumns)
	...
	protected List<String> readNextLine() throws InputIterationException {
		...
		if (lineArray == null) {
			...
			for (String val : lineArray) {
				if(!excludedColumns.contains(i++)){ // changed: ignore excluded columns
