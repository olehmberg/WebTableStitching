...
public class ClusteringPerformance {
	...
	private Performance transitiveCorrespondencePerformance;
	...
	public int getNumTransitiveIncorrectCorrespondences() {
		if(transitiveCorrespondencePerformance==null) {
			return 0;
		} else {
			return transitiveCorrespondencePerformance.getNumberOfPredicted()-transitiveCorrespondencePerformance.getNumberOfCorrectlyPredicted();
		}
	}
	...
	/**
	 * @return the transitiveCorrespondencePerformance
	 */
	public Performance getTransitiveCorrespondencePerformance() {
		return transitiveCorrespondencePerformance;
	}
	
	/**
	 * @param transitiveCorrespondencePerformance the transitiveCorrespondencePerformance to set
	 */
	public void setTransitiveCorrespondencePerformance(Performance transitiveCorrespondencePerformance) {
		this.transitiveCorrespondencePerformance = transitiveCorrespondencePerformance;
	}
	...
	public ClusteringPerformance(Map<String, Performance> performanceByCluster, boolean isInverse) {
		this.performanceByCluster = performanceByCluster;
		if(isInverse) {
			calculatePerformanceInverse();
		} else {
			calculatePerformance();
		}
	}
	...
	protected void calculatePerformanceInverse() {
		int correctSum = 0;
    	int predictedSum = 0;
    	int totalSum = 0;
    	...
    	for(String key : performanceByCluster.keySet()) {
    		...
    		// calculate weighted average
    		double weight = (p.getNumberOfPredicted() / (double)totalCors);
//    		double weightGs = (p.getNumberOfCorrectTotal() / (double)totalCors);
    		rec += p.getRecall() * weight; 
    		pr += p.getPrecision() * weight;
    	...
    	macroAverage = new IRPerformance(precisionSum/(double)performanceByCluster.size(), recallSum/(double)performanceByCluster.size());
...
//    	}
    	...
    	sb.append("Pair-wise Correspondences (Transitive)\n");
    	if(transitiveCorrespondencePerformance!=null) {
	    	p = transitiveCorrespondencePerformance;
	    	sb.append(String.format("\t%d incorrect correspondence(s) / %d total\n", p.getNumberOfPredicted()-p.getNumberOfCorrectlyPredicted(), p.getNumberOfPredicted()));
	    	sb.append(String.format("\t%s %.10f\n", org.apache.commons.lang3.StringUtils.rightPad("Precision:",10), p.getPrecision()));
	    	sb.append(String.format("\t%s %.10f\n", org.apache.commons.lang3.StringUtils.rightPad("Recall:",10), p.getRecall()));
	    	sb.append(String.format("\t%s %.10f\n", org.apache.commons.lang3.StringUtils.rightPad("F1:",10), p.getF1()));
    	} else {
