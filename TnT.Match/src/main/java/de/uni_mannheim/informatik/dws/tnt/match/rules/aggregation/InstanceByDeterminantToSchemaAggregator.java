 ...
 *
 */
public class InstanceByDeterminantToSchemaAggregator {
	...
	public InstanceByDeterminantToSchemaAggregator(int minVotes, boolean noFiltering, boolean uncertainOnly, boolean mustIncludeDependant, double minSimilarity) {
	...
	public ResultSet<Correspondence<MatchableTableColumn, MatchableTableRow>> aggregate(
			ResultSet<Correspondence<MatchableTableRow, MatchableTableKey>> correspondences,
			final ResultSet<Correspondence<MatchableTableColumn, MatchableTableRow>> determinantCorrespondences,
			DataProcessingEngine proc) {
		
		// co-group the instance correspondences and their schema correspondences by table combination
//		Function<Collection<Integer>, Correspondence<MatchableTableRow, MatchableTableKey>> recordToTableIds = new Function<Collection<Integer>, Correspondence<MatchableTableRow,MatchableTableKey>>() {
//			
//			@Override
//			public Collection<Integer> execute(Correspondence<MatchableTableRow, MatchableTableKey> record) {
//				if(record.getFirstRecord().getTableId()>record.getSecondRecord().getTableId()) {
//					System.out.println("Wrong direction!");
//				}
//				
//				List<Integer> keyValues = Q.toList(record.getFirstRecord().getTableId(), record.getSecondRecord().getTableId());
//				return keyValues;
//			}
//		};
//		Function<Collection<Integer>, Correspondence<MatchableTableColumn, MatchableTableRow>> correspondenceToTableIds = new Function<Collection<Integer>, Correspondence<MatchableTableColumn,MatchableTableRow>>() {
//			
//			@Override
...
//			}
//		};
//		RecordMapper<Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>>, Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>>> recordWithSchemaMapper = new RecordMapper<Pair<Iterable<Correspondence<MatchableTableRow,MatchableTableKey>>,Iterable<Correspondence<MatchableTableColumn,MatchableTableRow>>>, Pair<Iterable<Correspondence<MatchableTableRow,MatchableTableKey>>,Iterable<Correspondence<MatchableTableColumn,MatchableTableRow>>>>() {
//			
//			@Override
//			public void mapRecord(
//					Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>> record,
//					DatasetIterator<Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>>> resultCollector) {
//				resultCollector.next(record);
//			}
//		};
//		ResultSet<Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>>> recordsWithSchema = proc.coGroup(correspondences, determinantCorrespondences, recordToTableIds, correspondenceToTableIds, recordWithSchemaMapper);
//		
//		// group the correspondences by the table combination and the determinant combination
//		RecordKeyValueMapper<Collection<Object>, Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>>, Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>>> groupByTableAndDetermiant = new RecordKeyValueMapper<Collection<Object>, Pair<Iterable<Correspondence<MatchableTableRow,MatchableTableKey>>,Iterable<Correspondence<MatchableTableColumn,MatchableTableRow>>>, Pair<Iterable<Correspondence<MatchableTableRow,MatchableTableKey>>,Iterable<Correspondence<MatchableTableColumn,MatchableTableRow>>>>() {
//			
//			@Override
//			public void mapRecord(
//					Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>> record,
//					DatasetIterator<Pair<Collection<Object>, Pair<Iterable<Correspondence<MatchableTableRow, MatchableTableKey>>, Iterable<Correspondence<MatchableTableColumn, MatchableTableRow>>>>> resultCollector) {
//				
//				// input: all instance and schema correspondences for a pair of tables
//				// output: grouped by determiant combination
//
//				LinkedList<Correspondence<MatchableTableRow, MatchableTableKey>> instanceCors;
//				LinkedList<Correspondence<MatchableTableColumn, MatchableTableRow>> schemaCors;
//				
//				
//				
//				if(record.getFirstRecord().getTableId()>record.getSecondRecord().getTableId()) {
...
//			}
//		};
//		proc.groupRecords(recordsWithSchema, groupByTableAndDetermiant);
...
//					System.out.println(String.format("%s  <->  %s", StringUtils.join(record.getFirstRecord().get(Q.toPrimitiveIntArray(leftKey)),","), StringUtils.join(record.getSecondRecord().get(Q.toPrimitiveIntArray(rightKey)),",")));
		...
		RecordMapper<Group<Collection<Object>, Correspondence<MatchableTableRow, MatchableTableKey>>, Correspondence<MatchableTableColumn, MatchableTableRow>> voteMapper = new RecordMapper<Group<Collection<Object>,Correspondence<MatchableTableRow,MatchableTableKey>>, Correspondence<MatchableTableColumn,MatchableTableRow>>() {
			...
			public void mapRecord(Group<Collection<Object>, Correspondence<MatchableTableRow, MatchableTableKey>> record,
				...
				List key =((List) (new ArrayList<>(record.getKey()))); // not nice ...
				int t1 = (int)key.get(0);
				int t2 = (int)key.get(1);
				List k1 = (List)key.get(2);
				List k2 = (List)key.get(3);
				
				
				// map for schema correspondences
				Map<MatchableTableColumn, MatchableTableColumn> determinantMap = new HashMap<>();
				
				for(Correspondence<MatchableTableColumn, MatchableTableRow> scor : determinantCorrespondences.get()) {
					// filter out the correspondences of other table combinations
					if(scor.getFirstRecord().getTableId()==t1 && scor.getSecondRecord().getTableId()==t2) {
						
						// filter out the correspondences of other determinant combinations
						if(k1.contains(scor.getFirstRecord().getColumnIndex()) && k2.contains(scor.getSecondRecord().getColumnIndex())) {
							
							determinantMap.put(scor.getFirstRecord(), scor.getSecondRecord());
				...
				for(Correspondence<MatchableTableRow, MatchableTableKey> cor : record.getRecords().get()) {
					...
					for(Pair<MatchableTableColumn, MatchableTableColumn> vote : votes) {
						...
						if(determinantMap.containsKey(vote.getFirst())) {
							// this is a vote for a determinant
							// only use it if it is consistent with how the determinants were mapped when creating the record links
							if(!determinantMap.get(vote.getFirst()).equals(vote.getSecond())) {
								continue;
							}
