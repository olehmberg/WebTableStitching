 ...
 * 
 * Considers multiple possible mappings between the attributes.
 * 
 * @author Oliver Lehmberg (oli@dwslab.de)
 *
 */
public class KeyByMultiSchemaCorrespondenceBlocker<T> extends KeyBySchemaCorrespondenceBlocker<T> {

	/**
	 * 
	 */
	public KeyByMultiSchemaCorrespondenceBlocker(boolean onlyFullKeyMatches) {
		super(onlyFullKeyMatches);
	}
	...
	public ResultSet<Correspondence<MatchableTableKey, MatchableTableColumn>> runBlocking(
		...
		RecordKeyValueMapper<Pair<Integer, Integer>, Pair<MatchableTableKey, Pair<MatchableTableKey, Group<Pair<Integer, Integer>, Correspondence<MatchableTableColumn, T>>>>, Correspondence<MatchableTableKey, MatchableTableColumn>> groupByTablePair = new RecordKeyValueMapper<Pair<Integer,Integer>, Pair<MatchableTableKey,Pair<MatchableTableKey,Group<Pair<Integer,Integer>,Correspondence<MatchableTableColumn,T>>>>, Correspondence<MatchableTableKey,MatchableTableColumn>>() {
			...
			public void mapRecord(
				...
				// get the keys that we have to validate
				...
				// we have multiple possible mappings between the keys
				// so we have to find all possible assignments
				
				Map<MatchableTableColumn, Collection<Correspondence<MatchableTableColumn, T>>> leftToRight = Q.group(record.getSecond().getSecond().getRecords().get(), new Func<MatchableTableColumn, Correspondence<MatchableTableColumn, T>>(){

					@Override
					public MatchableTableColumn invoke(Correspondence<MatchableTableColumn, T> in) {
						return in.getFirstRecord();
					}});
				
				// recursively generate the possible assignments
				Collection<Collection<Correspondence<MatchableTableColumn, T>>> assignments = listAssignments(new LinkedList<>(leftToRight.keySet()), record.getSecond().getSecond().getRecords().get(), new LinkedList<Correspondence<MatchableTableColumn, T>>());
				
//				if(assignments.size()>1) {
//					StringBuilder sb = new StringBuilder();
//					for(Collection<Correspondence<MatchableTableColumn, T>> correspondences : assignments) {
//						for(Correspondence<MatchableTableColumn, T> cor : correspondences) {
//							sb.append(String.format("%s/%s, ", cor.getFirstRecord(), cor.getSecondRecord()));
//						}
//						sb.append("\n");
//					}
//					
...
//				}
				...
				// for every possible combination of correspondences, validate if the keys are completely mapped
				for(Collection<Correspondence<MatchableTableColumn, T>> correspondences : assignments) {
...
//				}
	...
	protected Collection<Collection<Correspondence<MatchableTableColumn, T>>> listAssignments(
			Queue<MatchableTableColumn> leftKey, 
			Collection<Correspondence<MatchableTableColumn, T>> correspondences, 
			Collection<Correspondence<MatchableTableColumn, T>> assignmentsSoFar) {

		// possibly the least memory-efficient solution 
		
		MatchableTableColumn nextLeft = leftKey.poll();
		
		if(nextLeft==null) {
			return Q.toList(assignmentsSoFar);
		} else {
			Map<MatchableTableColumn, Collection<Correspondence<MatchableTableColumn, T>>> leftToRight = Q.group(correspondences, new Func<MatchableTableColumn, Correspondence<MatchableTableColumn, T>>(){
			...
			Collection<Correspondence<MatchableTableColumn, T>> possibleAssignments = leftToRight.get(nextLeft);
			
			Collection<Collection<Correspondence<MatchableTableColumn, T>>> results = new HashSet<>();
			
			if(possibleAssignments==null) {
				for(Collection<Correspondence<MatchableTableColumn, T>> result : listAssignments(new LinkedList<>(leftKey), correspondences, assignmentsSoFar)) {
					results.add(result);
				}	
			} else {
				for(Correspondence<MatchableTableColumn, T> assignment : possibleAssignments) {
					LinkedList<Correspondence<MatchableTableColumn, T>> nextAssignment = new LinkedList<>(assignmentsSoFar);
					nextAssignment.add(assignment);
	
					Collection<Correspondence<MatchableTableColumn, T>> remaining = new LinkedList<>();
					for(Correspondence<MatchableTableColumn, T> cor : correspondences) {
						if(!cor.getSecondRecord().equals(assignment.getSecondRecord())) {
							remaining.add(cor);
						}
					}
					
					for(Collection<Correspondence<MatchableTableColumn, T>> result : listAssignments(new LinkedList<>(leftKey), remaining, nextAssignment)) {
						results.add(result);
					}				
				}
			}
