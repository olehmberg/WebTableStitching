...
public class EntityLabelDetection extends TnTTask {
	...
	public void match() throws Exception {
		...
		for(File f : webFile.listFiles()) {
			...
			noContext.identifyKey(0.001, true);
			
			if(noContext.hasKey()) {
				t.setKeyIndex(noContext.getKeyIndex() + (t.getColumns().size() - noContext.getColumns().size()));
				System.out.println(String.format("Entity-Label Column for '%s' is '%s'", f.getName(), t.getSchema().get(t.getKeyIndex()).getHeader()));
			} else {
				t.setKeyIndex(-1);
			}
