 ...
 *
 */
public class SchemaToSingleColumnCombinationAggregator {
	
	private int minColumns = 1;
	
	public SchemaToSingleColumnCombinationAggregator(int minColumns) {
	...
	public ResultSet<Correspondence<MatchableTableKey, MatchableTableColumn>> aggregate(ResultSet<Correspondence<MatchableTableColumn, MatchableTableRow>> correspondences,
		...
		// group schema correspondences by left table
...
//				}
				...
				resultCollector.next(new Pair<>(Q.toList(record.getFirstRecord().getTableId()), record));
...
//				}
				...
				if(cols1.size()>=minColumns) {
					resultCollector.next(new Correspondence<MatchableTableKey, MatchableTableColumn>(k1, new MatchableTableKey(-1, null), 1.0, null));
				}
			}
		};
