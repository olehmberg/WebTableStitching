...
public class KnowledgeBaseTest extends TestCase {
	/**
	 * Test method for {@link de.uni_mannheim.informatik.dws.t2k.match.data.KnowledgeBase#loadKnowledgeBase(java.io.File, de.uni_mannheim.informatik.dws.t2k.index.IIndex, boolean, SurfaceForms)}
	 * @throws IOException 
	 */
	public void testLoadKnowledgeBase() throws IOException{
    	...
    	assertNotNull(sf);
		kb = KnowledgeBase.loadKnowledgeBase(new File("src\\test\\resources\\kbtables\\"), index, false, sf);
		
//		check for number of records and schema
		assertEquals(10, kb.getRecords().getSize());
		assertEquals(4, kb.getSchema().getSize());	
