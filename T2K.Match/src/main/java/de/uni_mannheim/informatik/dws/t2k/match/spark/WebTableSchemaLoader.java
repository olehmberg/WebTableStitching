 ...
 *
 */
public class WebTableSchemaLoader implements RecordMapper<Table, MatchableTableColumn> {
	...
	public void mapRecord(Table record, DatasetIterator<MatchableTableColumn> resultCollector) {
    	for(TableColumn c : record.getSchema().getRecords()) {
    		MatchableTableColumn mc = new MatchableTableColumn(record.getTableId(), c);
