...
public class TripleGenerator {
	...
	public TripleGenerator(WebTables web, KnowledgeBase kb, DataProcessingEngine proc) {
		this.web = web;
		this.kb = kb;
		this.proc = proc;
	}
	
	public ResultSet<ExtractedTriple> run(ResultSet<Correspondence<MatchableTableRow, MatchableTableColumn>> instanceCorrespondences, ResultSet<Correspondence<MatchableTableColumn, MatchableTableRow>> schemaCorrespondences) {
					...
					if(cor.getSecondRecord().getColumnIndex()!=kb.getRdfsLabel().getColumnIndex()) {
						...
						for(Correspondence<MatchableTableRow, MatchableTableColumn> row : record.getFirst()) {
							...
							if(tableValue!=null) {
								...
								int kbColIndex = kb.getPropertyIndices().get(row.getSecondRecord().getTableId()).get(cor.getSecondRecord().getColumnIndex());
								Object kbValue = row.getSecondRecord().get(kbColIndex);
								if(kbValue!=null) {
...
//									double sim = comparator.compare(row.getFirstRecord(), row.getSecondRecord(), cor.getFirstRecord().getColumnIndex(), cor.getSecondRecord().getColumnIndex());
		...
		ResultSet<ExtractedTriple> triples = proc.coGroup(instanceCorrespondences, schemaCorrespondences, groupInstanceCorByTable, groupSchemaCorByTable, resultMapper);
	...
	protected void countNewTriples(ResultSet<ExtractedTriple> triples) {
		RecordKeyValueMapper<Boolean, ExtractedTriple, ExtractedTriple> groupByIsNew = new RecordKeyValueMapper<Boolean, ExtractedTriple, ExtractedTriple>() {
			...
			@Override
			public void mapRecord(ExtractedTriple record, DatasetIterator<Pair<Boolean, ExtractedTriple>> resultCollector) {
		...
		ResultSet<Pair<Boolean, Integer>> counts = proc.aggregateRecords(triples, groupByIsNew, new CountAggregator<Boolean,ExtractedTriple>());
	...
	protected void countCorrectTriples(ResultSet<ExtractedTriple> triples) {
		...
		ResultSet<Pair<Boolean, Integer>> counts = proc.aggregateRecords(triples, groupByIsCorrect, new CountAggregator<Boolean,ExtractedTriple>());
